...
static int swc_pack_gray(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	uint32_t w = src->format->width;
	uint32_t h = src->format->height;
	uint32_t y;
...
static int swc_unpack_yuv24(struct pitcher_buffer *src,
			   struct pitcher_buffer *dst)
{
	uint32_t w = src->format->width;
	uint32_t h = src->format->height;
...
static int swc_pack_yuv24(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	uint32_t w = src->format->width;
	uint32_t h = src->format->height;
	uint32_t y;
...
static int swc_unpack_y16(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	uint32_t w = src->format->width;
	uint32_t h = src->format->height;
	uint32_t y;
...
static int swc_pack_y16(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	uint32_t w = src->format->width;
	uint32_t h = src->format->height;
	uint32_t y;
...
int pitcher_sw_unpack(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	...
	case PIX_FMT_NV12:
		...
		break;
	case PIX_FMT_GRAY:
		ret = swc_unpack_gray(src, dst);
		break;
	case PIX_FMT_YUV24:
		ret = swc_unpack_yuv24(src, dst);
		break;
	default:
		break;
	}
...
int pitcher_sw_pack(struct pitcher_buffer *src, struct pitcher_buffer *dst)
{
	...
	case PIX_FMT_NV12:
		...
		break;
	case PIX_FMT_GRAY:
		ret = swc_pack_gray(src, dst);
		break;
	case PIX_FMT_YUV24:
		ret = swc_pack_yuv24(src, dst);
		break;
	default:
		break;
	}
